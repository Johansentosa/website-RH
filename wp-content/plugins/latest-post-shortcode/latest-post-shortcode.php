...
class Latest_Post_Shortcode {
	 ...
	 * @var array
	 */
	var $tax_positions = array();

	/**
	 * Pugin tags replaceable.
	 * @var array
	 */
	var $replaceable_tags = array( 'date', 'title', 'text', 'image', 'read_more_text', 'author', 'category', 'tags' );
	...
	function tile_pattern_setup() {
		...
		if ( ! empty( $filtered_tax ) ) {
			foreach ( $filtered_tax as $key => $value ) {
				array_push( $this->replaceable_tags, $key );
			}
		}
		$this->replaceable_tags = array_unique( $this->replaceable_tags );
	}
						...
						<div class="lps_tabs">
							...
							<div id="tabs-1">
								<table width="100%" cellspacing="0" cellpadding="2">
									...
									<tr>
										<td class="lps_title_td" valign="top"><?php esc_html_e( 'Author ID', 'lps' ); ?></td>
										<td>
											<input type="text" name="lps_author_id" id="lps_author_id" onchange="lps_preview_configures_shortcode()" placeholder="<?php esc_attr_e( 'Separate IDs with comma', 'lps' ); ?>" />
										</td>
										<td colspan="2"><?php esc_attr_e( '(will show only objects with the selected author IDs)', 'lps' ); ?></td>
									</tr>
								...
								<table width="100%" cellspacing="0" cellpadding="2">
									...
									<tr>
										<td>
											...
											if ( ! empty( $alltax ) ) :
												foreach ( $alltax as $slug => $name ) :
												?>
													<div>
														<label><input type="checkbox" name="lps_show_extra[]" id="lps_show_extra_<?php echo esc_attr( $slug ); ?>" value="<?php echo esc_attr( $slug ); ?>" onclick="lps_preview_configures_shortcode();" class="lps_show_extra lps-is-taxonomy" /> <b><?php esc_html_e( 'Show ', 'lps' ); ?> <?php echo esc_html( $name ); ?></b>,</label>
														<?php if ( 'category' === $slug ) : ?>
															<label><input type="checkbox" name="lps_show_extra[]" id="lps_show_extra_hide_uncategorized_<?php echo esc_attr( $slug ); ?>" value="hide_uncategorized_<?php echo esc_attr( $slug ); ?>" onclick="lps_preview_configures_shortcode();" class="lps_show_extra" /> <b><?php esc_html_e( 'Do not display Uncategorized term', 'lps' ); ?></b>,</label>
														<?php endif; ?>
													</div>
													<div id="lps_show_extra_<?php echo esc_attr( $slug ); ?>_pos_wrap">
														<table>
															<tr>
																<td class="lps_title_td"><?php esc_html_e( 'Display position', 'lps' ); ?></td>
																<td>
																	<label class="show-options"><input type="radio" name="lps_show_extra_pos_<?php echo esc_attr( $slug ); ?>" id="lps_show_extra_taxpos_<?php echo esc_attr( $slug ); ?>_default" value="" checked="checked" onclick="lps_preview_configures_shortcode();" class="lps_show_extra" /><?php esc_html_e( 'default', 'lps' ); ?>,</label>
																	<?php foreach ( $this->tax_positions as $pos => $pos_title ) : ?>
																		<label class="show-options"><input type="radio" name="lps_show_extra_pos_<?php echo esc_attr( $slug ); ?>" id="lps_show_extra_taxpos_<?php echo esc_attr( $slug ); ?>_<?php echo esc_attr( $pos ); ?>" value="taxpos_<?php echo esc_attr( $slug ); ?>_<?php echo esc_attr( $pos ); ?>" onclick="lps_preview_configures_shortcode();" class="lps_show_extra" /><?php echo esc_html( $pos_title ); ?>,</label>
																	<?php endforeach; ?>
																</td>
															</tr>
														</table>
		...
		if ( ! empty( $args['status'] ) ) {
			...
			if ( in_array( 'private', $qargs['post_status'] ) ) {
				if ( ! is_user_logged_in() ) {
					$pkey = array_search( 'private', $qargs['post_status'] );
					if ( false !== $pkey ) {
			...
			foreach ( $posts as $post ) {
				...
				if ( $use_custom_markup ) {
					...
					if ( in_array( 'excerpt', $extra_display ) || in_array( 'content', $extra_display ) || in_array( 'content-small', $extra_display ) || in_array( 'excerpt-small', $extra_display ) ) {
						...
						} elseif ( in_array( 'content', $extra_display ) ) {
	...
	function positions_from_extra( $show_extra = '', $tile_pattern = '', $args = array(), $extra_display = array() ) {
		if ( in_array( 'date', $extra_display ) ) {
			if ( in_array( 'title', $extra_display ) ) {
		...
		if ( ! empty( $show_extra ) ) {
			foreach ( $show_extra as $extra_tag ) {
				if ( substr_count( $extra_tag, 'taxpos_' ) ) {
					...
					if ( ! empty( $matches[1][0] ) && in_array( $matches[1][0], $show_extra ) && ! empty( $matches[2][0] ) ) {
		...
		foreach ( $this->replaceable_tags as $tag ) {
