...
class SupsysticSlider_Bx_Module extends Rsc_Mvc_Module implements SupsysticSlider_Slider_Interface
{
    ...
    public function enqueueJavascript()
    {
        ...
        wp_enqueue_script(
            ...
            $this->getLocationUrl() . '/assets/js/jquery.fitvids.js',
            array(),
            '1.0.0',
            true
        );
    ...
    public function render($slider)
    {
        foreach($slider->images as $key => $value) {
			...
			$slider->images[$key]->attachment['description_raw'] = strip_tags(htmlspecialchars_decode($slider->images[$key]->attachment['description'], ENT_QUOTES));
        }

        /* Multiple rows of thumbnails */
        $chunkClount = 1;

        if(isset($slider->settings['general']['rows']) && (int)$slider->settings['general']['rows']) {
            $chunkClount = $slider->settings['general']['rows'];
        }
        $slider->thumbnails = array_chunk($slider->entities, $chunkClount);

        return $this->getEnvironment()->getTwig()->render($this->getSliderTemplate(), array('slider' => $slider));
    }
