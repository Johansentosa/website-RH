...
if ( ! function_exists( 'alx_setup' ) ) {
	...
	function alx_setup() {	
		...
		add_image_size( 'thumb-large', 720, 220, true );
...
// Ngedit Excerpt disini cuy.
if(!function_exists('custom_excerpt')){
	function custom_excerpt($txt){
		$text_length = 50;
		$text = substr($txt, 0, $text_length);
		$more = (strlen($text) == $text_length) ? '...' : '';
		return $text.$more;
	}
}
add_filter( 'the_excerpt', 'custom_excerpt', $txt);

if(!function_exists('custom_title')){
	function custom_title($txt){
		$text_length = 50;
		$text = substr($txt, 0, $text_length);
		$more = (strlen($text) == $text_length) ? '...' : '';
		return $text.$more;
...
// add_filter( 'the_title', 'custom_title', $txt);
